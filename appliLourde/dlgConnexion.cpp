...
void DlgConnexion::on_butConnexion_clicked()
{
    ...
    QString qsReq="SELECT typePersonnel,numeroPersonnel FROM Personnel WHERE username = '"+ qsUser + "' AND mdp = '"+qsMdp + "'";
    //qDebug()<<qsReq;
    ...
    if(reqConnexion.exec())
    {
       ...
       m_iDUser = reqConnexion.value(1).toString();
       accept();
...
QString DlgConnexion::getID()
{
    qDebug()<<"QString DlgConnexion::getID()"<<endl;
    return m_iDUser;
}
